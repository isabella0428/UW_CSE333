...
int InsertHashTable(HashTable table,
  ...
  do{
    if(((HTKeyValue *)payload)->key == newkeyvalue.key){
      oldkeyvalue->key = ((HTKeyValue *)payload)->key;
      oldkeyvalue->value = ((HTKeyValue *)payload)->value;
      ((HTKeyValue *)payload)->value = newkeyvalue.value;
      table->num_elements += 1;
      return 2;
    }
    LLIteratorNext(ll_iter);
    LLIteratorGetPayload(ll_iter, &payload);
  }while (payload!= NULL);
...
int LookupHashTable(HashTable table,
  ...
  int block_to_insert = key % (table->num_buckets);
  LinkedList lookUpChain = table->buckets[block_to_insert];
  if(NumElementsInLinkedList(lookUpChain) == 0){
    return 0;
  }

  LLIter iterator = LLMakeIterator(lookUpChain, 0);
  LLPayload_t payload;
  LLIteratorGetPayload(iterator, &payload);

  do{
    if(((HTKeyValue *)payload)->key == key){
      keyvalue->key = ((HTKeyValue *)payload)->key;
      keyvalue->value = ((HTKeyValue *)payload)->value;
      return 1;
    }
...
int RemoveFromHashTable(HashTable table,
  ...
  LLIter iterator = LLMakeIterator(RemoveChain, 0);
  LLPayload_t payload;
  LLIteratorGetPayload(iterator, &payload);
...
int HTIteratorNext(HTIter iter) {
  ...
  HWSize_t old_size;
  HTIter new_iter;
  old_size = (iter->ht)->num_elements;
  (iter->ht)->num_elements = iter->bucket_num - 1;
  new_iter= HashTableMakeIterator(iter->ht);
  if(new_iter == NULL){
    return 0;
  }
  iter = new_iter;
  (iter->ht)->num_elements = old_size;
...
int HTIteratorPastEnd(HTIter iter) {
  ...
  for (i = iter->bucket_num; i < iter->bucket_num; i++)
  {
